...
def make_dir(path):
    """
    create a directory is there isn't one already
    :param path:
    :return:
    """
    try:
...
def prepare_dataset(questions, answers):
    # create path to store all the train and test encoders and decoders
    make_dir(config.PROCESSED_PATH)

    # select random conversations to make the test set.
...
def basic_tokenizer(line, normalize_digits=True):
    """
    A basic tokenizer.  I can probably replace this with nltk!
    :param line:
...
def token2id(data, mode):
    """
    Convert all tokens in the daet into their corresponding index in the vocabulary.
    :param data:
    :param mode:
    :return:
    """
...
def prepare_raw_data():
    print('Preparing raw data into training and testing sets ...')
    id2line = get_lines()
    convos = get_convos()
...
def load_data(enc_filename, dec_filename, max_training_size=None):
    ...
    encode, decode = encode_file.readlines(), decode_file.readline()
...
def get_batch(data_bucket, bucket_id, batch_size=1):
    ...
    for length_id in range(decoder_size):
        ...
        for batch_id in range(batch_size):
            # Set the mask to 0 is the corresponding target in a PAD symbol.
